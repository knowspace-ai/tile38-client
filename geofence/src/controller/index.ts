...
export const healthCheck = async (req: Request, res: Response): Promise<any> => {
   try {
      ...
      return res.status(400).json({ message: `Error occurred:` + err?.message || err?.data });
   }
};
...
export const setGeofencing = async (req: Request, res: Response): Promise<any> => {
   try {
      ...
      return res.status(400).json({ message: `setGeofencing Error occurred:` + err?.message || err?.data });
   }
};

export const setCommand = async (req: Request, res: Response): Promise<any> => {
   try {
      ...
      return res.status(status_code).json({ data, message: data?.err || 'success', status: data?.ok });
   } catch (err: any) {
      logger.error(`command Error message:` + err?.message || err?.data);
      return res.status(400).json({ message: `command Error occurred:` + err?.message || err?.data });
   }
};
...
export const setUser = async (req: Request, res: Response): Promise<any> => {
   try {
      ...
      return res.status(400).json({ message: `setUser Error occurred:` + err?.message || err?.data });
   }
};
